...
TEXTS = {
    "es": {
        ...
        "preview_footer_fischer": "➡️ El alumno deberá responder dibujando la molécula en el editor JSME.",
        ...
        "start_standardization": "Normalizar con JSME",
    ...
    "en": {
        ...
        "preview_footer_fischer": "➡️ The student must answer drawing the correct molecule in JSME editor.",
        ...
        "start_standardization": "Normalize with JSME",
...
def render_fischer_app(lang="en"):
    ...
    if st.session_state.get("f_jsme_in") and jsme_out:
        try:
            ...
            if (
                ...
                # Limpiar y pasar a la siguiente pregunta pendiente (si existe)
                st.session_state.f_jsme_in = None
                st.session_state.f_curr_req = None
                start_jsme_loop()
                st.rerun()
    ...
    with list_c:
        ...
        pending = any(q['smiles_normalized'] is None for q in st.session_state.f_questions)
        if st.session_state.f_questions:
            if pending:
                if st.button(texts["start_standardization"], type="primary",
                icon=":material/rocket_launch:", use_container_width=True):
                    idx = next(i for i, q in enumerate(st.session_state.f_questions) if q['smiles_normalized'] is None)
                    req_id = str(uuid.uuid4())
                    st.session_state.jsme_input = json.dumps({'smiles': st.session_state.f_questions[idx]['smiles_raw'], 'id': req_id})
                    st.session_state.pending_request = {'id': req_id, 'name': st.session_state.f_questions[idx]['name'], 'original_index': idx}
                    st.session_state.f_is_processing = True
                    st.rerun()
            else:
                quiz = ET.Element("quiz")
                for q in st.session_state.f_questions:
                    qu = ET.SubElement(quiz, "question", type="pmatchjme")
                    ET.SubElement(ET.SubElement(qu, "name"), "text").text = q['name']
                    w_txt = f"<p style='color:red;'><i>{texts['stereo_warning_moodle']}</i></p>" if q['has_undefined'] else ""
                    txt = f"<p>{texts['question_text_moodle']}</p>{w_txt}<p style='text-align:center;'><img src='data:image/png;base64,{q['img']}'></p>"
                ...
                st.download_button(texts["download_xml"], data=ET.tostring(quiz, encoding="utf-8", xml_declaration=True), file_name="fischer_moodle.xml", mime="application/xml",
                type="primary", icon= ":material/download:", use_container_width=True)
            
            if st.button(texts["clear_all"], icon="🗑️", use_container_width=True):
                st.session_state.f_questions = []; st.session_state.f_error_msg = None; st.rerun()

            st.divider()
            for i, q in enumerate(st.session_state.f_questions):
                c1, c2, c3 = st.columns([1.5, 3, 0.5])
                with c1: st.image(f"data:image/png;base64,{q['img']}", width=150)
                with c2:
                    st.markdown(f"**{q['name']}**")
                    if q['has_undefined']: st.warning(texts["stereo_warning"])
                    if q['smiles_normalized']: st.caption(f"✅ `{q['smiles_normalized']}`")
                    else: st.info(texts["not_normalized"])
                with c3:
                    if st.button("🗑️", key=f"f_del_{i}"): st.session_state.f_questions.pop(i); st.rerun()
                st.divider()
        else:
