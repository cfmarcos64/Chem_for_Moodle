...
def canonicalize_smiles(smiles):
    """Uses RDKit to validate and canonicalize a SMILES string.
...
def get_smiles_from_name(name):
    """Queries the NCI CIR API to resolve a compound name to a SMILES string.
    Returns None on failure or HTTP error."""
    try:
...
def process_input_to_smiles(user_input):
    """Determines whether the input is a SMILES string or a compound name,
    then returns a canonicalized SMILES.  Returns None if resolution fails."""
    if not user_input:
        return None
    user_input = user_input.strip()

    # Heuristic: treat as SMILES if it contains typical SMILES characters or is long
    if any(c in user_input for c in "=#()[]") or len(user_input) > 25:
        return canonicalize_smiles(user_input)

    # Otherwise resolve by name via NCI CIR
    sm = get_smiles_from_name(user_input)
    return canonicalize_smiles(sm) if sm else None


def escape_smiles_for_xml(smiles):
    """Escapes characters that the Moodle pmatchjme engine treats as special.
    Applies double-escaping as required by the pmatchjme answer format."""
    if not smiles:
        return ""
    return (
        smiles
...
def generate_xml(questions, lang):
    ...
    buffer = io.BytesIO()
    buffer.write(ET.tostring(root, encoding="utf-8", xml_declaration=True))
...
def render_molecule_app(lang="en"):
    ...
    # --- Preview expander: sample Moodle question ---
    ...
    if "m_questions" not in st.session_state:
        st.session_state.m_questions = []
    if "m_jsme_in" not in st.session_state:
    ...
    def start_jsme_loop():
        ...
        idx = next(
            (i for i, q in enumerate(st.session_state.m_questions) if q.get("smiles_norm") is None),
            None,
        )
        if idx is not None:
    ...
    # --- Invisible JSME editor for SMILES normalisation ---
    ...
    if st.session_state.m_jsme_in and jsme_out:
        try:
            ...
            st.error(texts["jsme_processing_error"].format(e))

    # --- Two-column layout: input on the left, question list on the right ---
    ...
    with m_col:
        ...
        with t_bulk:
            ...
            uploaded_file = st.file_uploader(texts["section_bulk"], type=["csv", "xlsx"])
    ...
    with l_col:
        ...
        if qs:
            if pending:
                if st.button(
                    texts["start_standardization"].format(len(pending)),
                    type="primary",
            ...
            for i, q in enumerate(qs):
                ...
                with c2:
                    if st.button("🗑️", key=f"del_m_{i}", help=texts["delete_question"]):
